"""
================================================================================
Day 1 - 模型配置 | Model_config.py
================================================================================

【学习目标】
理解模型配置抽象：如何从环境变量加载配置，支持多 Provider 切换
...
2. 运行: python Model_config.py
3. 观察输出：模型配置信息（脱敏显示）

【预期输出】
ModelConfig(model=gpt-4o-mini, base_url=https://api.openai.com/v1, temp=0.7, max_tokens=2048, key=sk-xxxx...xxxx)

【验证标准】
□ 能看到模型配置信息
□ API Key 是脱敏显示的（前8位+...+后4位）
□ 尝试修改 .env 中的 MODEL_NAME，观察输出变化

【代码要点】
- ModelConfig.from_env(): 从环境变量加载配置
- get_client_kwargs(): 返回 OpenAI client 构造参数
...
if __name__ == "__main__":
    ...
    print(config)
    print(f"\nClient kwargs: {config.get_client_kwargs()}")
